...
<p><strong>Threshold τ 是什麼？</strong>閾值 $\\tau$ 是判斷某位候選人「算不算支持目前模型」的容忍距離。計算出每位候選人的殘差 $r_i$ 後，只有 $r_i &lt; \\tau$ 的人才能進入 Consensus Set。$\\tau$ 設太小 → 幾乎沒人達標，RANSAC 難以建立共識；$\\tau$ 設太大 → Outlier 也能混入，污染 Consensus Set，最終模型失準。在這個介面中，滑桿範圍 1–30 線性對應 $\\tau = 0.01$–$0.30$（單位是正規化後的 $Y$ 空間，值域 $[0, 1]$），因此滑桿設為 10 代表：「預測相容性分數與實際分數相差在 ±0.10 以內的候選人都算符合」。</p>
